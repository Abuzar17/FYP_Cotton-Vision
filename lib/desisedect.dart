...
class _CottonPredictionsPageState extends State<CottonPredictionsPage>
  ...
  Future<void> _predictImage() async {
    ...
    final uri = Uri.parse('http://192.168.13.209:8181/detectdisease');
    final request = http.MultipartRequest('POST', uri);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (yieldCategory == 'Unknown') {
          _showUnknownResultDialog();
          await getGeminiSolution(
              'Prediction result is unknown for cotton disease detection. The image may not clearly show a cotton plant or disease. Common cotton diseases include bacterial blight, fusarium wilt, and boll rot.');
        } else {
          Navigator.push(
            context,
            PageRouteBuilder(
        ...
        _showUnknownResultDialog();
        await getGeminiSolution(
            'Prediction error occurred in cotton disease detection API. The server responded with status code ${response.statusCode}.');
      }
    } catch (e) {
      print('Failed to predict: $e');
      setState(() {
      ...
      _showUnknownResultDialog();
      await getGeminiSolution(
          'Error occurred during the cotton disease prediction API call: $e');
    }
  }

  Future<void> getGeminiSolution(String problemDescription) async {
    final gemini = Gemini.instance;

    try {
      final response = await gemini.text(
        '''Problem related to cotton disease prediction: $problemDescription. 
        Please suggest a solution, considering common cotton diseases like bacterial blight, fusarium wilt, and boll rot. If the issue is related to image quality or API errors, provide steps to improve image quality or troubleshoot the API call. Keep the response concise and actionable.''',
        safetySettings: [
          SafetySetting(
      ...
      if (response != null && response.content?.parts?.isNotEmpty == true) {
        final solution = response.content!.parts!.last.toString() ??
            'No solution available.';
        print("Gemini's response: $solution");
        _showSolutionDialog(solution);
      } else {
        print("Gemini response is null or empty");
        _showSolutionDialog(
            'No solution received. Please try again.');
      }
    } catch (e) {
      print("Error fetching solution from : $e");
      _showSolutionDialog('Failed to fetch solution: $e');
    }
  }
        ...
        title: Row(
          children: [
            ...
            DefaultTextStyle(
              ...
              child: AnimatedTextKit(
                animatedTexts: [
                  WavyAnimatedText('Suggested Solution'),
                ],
                isRepeatingAnimation: true,
              ),
            ),
